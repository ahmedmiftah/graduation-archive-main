...
watch(selectedDegree, (newVal) => {
    router.get('/statistics', { degree_level: newVal }, { replace: true, preserveState: true, preserveScroll: true });
});

// Chart data based on filtered departments
const chartData = computed(() => filteredDepartments.value.map((d) => ({ status_name: d.name, count: d.project_count })));
</script>
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="printable-area flex h-full flex-1 flex-col gap-6 p-4" dir="rtl">
            <h1 class="mb-4 text-2xl font-bold text-gray-900 dark:text-gray-100">إحصائيات المشاريع حسب الأقسام</h1>
            ...
            <div v-if="isSuperAdmin" class="mb-4">
                ...
                <select id="dept-select" v-model="selectedDept" class="rounded border px-2 py-1">
            ...
            <div class="mb-4">
                ...
                <select id="degree-select" v-model="selectedDegree" class="rounded border px-2 py-1">
